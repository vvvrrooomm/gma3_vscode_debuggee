...
local function getGma3Children(object)
	local gma3children = {}

	local i = 1;
	while object[i] do
		local childReference = object[i];
		if childReference.name then
			gma3children[childReference.name] = childReference;
		else
			gma3children[i] = childReference;
		end
		i = i + 1;
	end
...
local function getGma3Properties(object)
	local gma3properties = {}
	if IsObjectValid(object) then
		for i = 0, (object:PropertyCount() - 1) do
			local name  = 'UNKNOWN_NAME';
			local value = 'UNKNOWN_VALUE';

			if object:PropertyName(i) then
				name = tostring(object:PropertyName(i))
			end
			if object:Get(object:PropertyName(i)) then
				value = tostring(object:Get(object:PropertyName(i)))
			end

			gma3properties[name] = value;
		end
	end
	return gma3properties;
end
...
local function searchFileRecursive(path, searchedFile)
	for file in lfs.dir(path) do
		if file ~= "." and file ~= ".." then
			local currentPath = path..'/'..file
			if file == searchedFile then
				return currentPath;
			end
			local attr = lfs.attributes (currentPath)
			assert (type(attr) == "table")
			if attr.mode == "directory" then
				local resPath = searchFileRecursive(currentPath, searchedFile);
				if resPath then
					return resPath;
				end
			end
...
local function getPathFromFileName(src)
	-- file referencing is a bit tricky since we only get the filename, not full path from debug_getinfo
	-- we need to do some tricks...

	-- search in breakpoints
	local foundPath
	local maxMatchCount = 0
	for bppath, _ in pairs(breakpointsPerPath) do
		local splitted = splitChunkName(bppath)
		local splitedReqPath = splitChunkName(src)
	...
	if dumpGMA3 then 
		if foundPath then
			Echo("Path found in breakpointsPerPath: "..foundPath)
		else
			Echo("Path not found in breakpointsPerPath:"..src)
			gma3_helpers:dump(breakpointsPerPath)
		end
	end

	-- search in sourceBasePath
	local path = foundPath or Path.toAbsolute(sourceBasePath, src)
	if not file_exists(path) and src ~= "=[C]" then
		-- search file recursive in sourceBasePath
		local splitedReqPath = splitChunkName(src)
		local reducedToFileName = splitedReqPath[#splitedReqPath]
		local foundFilePath = searchFileRecursive(sourceBasePath, reducedToFileName)
		if foundFilePath then
			path = foundFilePath;
		end
		if not file_exists(path) then
			debuggee.print("warning", "File doesn't exist:",path)
		end
	end
	return path
end
...
local function logToDebugConsole(output, category)
    ...
    sendFully('Content-Length: ' .. #dumpBody .. '\r\n\r\n' .. dumpBody)
end
...
local function createPureBreaker()
	...
	breakpointsPerPath = {} -- reset

	local function chunkNameToPath(chunkname,dumpGMA3)
		local cached = chunknameToPathCache[chunkname]
		if cached then
			if dumpGMA3 then
				Echo("chunkNameToPath would return cached but this is ignored in dumpGMA3 mode")
			else
				return cached
			end
		end
		...
		for path, _ in pairs(breakpointsPerPath) do
			...
			if dumpGMA3 then
				...
				Echo("MatchCount -> "..tostring(count))
			end
			if (count > maxMatchCount) then
				maxMatchCount = count
				foundPath = path
	...
	local function hookfunc()
		...
		entered = true
		if dumpGMA3 then
			Echo("lua debugger: line event hook entered.")
		end
		...
		if info then
			...
			if bpSet and bpSet[info.currentline] then
				...
				if dumpGMA3 then
					...
					Echo("chunkNameToPath result = "..chunkNameToPath(info.source,true))
		...
		setBreakpoints = function(path, lines)
			...
			for _,ln in pairs(lines) do
				if file_exists(path) then
					...
					debuggee.print("error", "Breakpoint path wrong:",path, "(line "..ln..")")
				end
			end
			return verifiedLines
		end,
...
local function recvMessage()
	...
	if (string.sub(header, 1, 16) ~= 'Content-Length: ') then
		error('wrong header: ' .. header)
	end
	local pos,pos_end = string.find(header,'%d+', 16)
	if not pos then
		error('Invalid protocol. length not found in header: ' .. header)
	end
	
	local length = tonumber(header:sub(pos, pos_end))+2-- 4 is the length of \r\n\r\n
	if not length then
...
function debuggee.start(jsonLib, config)
	...
	debuggee.print("output", "workspaceRoot: ", sourceBasePath)
...
function handlers.stackTrace(req)
	...
	for i = firstFrame, lastFrame do
		...
		local path = getPathFromFileName(src)
...
function handlers.variables(req)
	...
	if (varRef >= 1000000) then
		...
		elseif type(var) == 'userdata' then
			...
			local gma3_properties = getGma3Properties(var)
			for propName, propContent in pairs(gma3_properties) do
				addVar(propName,propContent)
			end
			local gma3_children = getGma3Children(var)
			addVar("(children)",gma3_children)
		end
